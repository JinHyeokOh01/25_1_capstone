...
low_freq_count = 500
gpu_milli_data = df['gpu_milli'].values
fft_signal = np.fft.rfft(gpu_milli_data)
fft_signal[low_freq_count:] = 0
df['gpu_milli_fft'] = np.fft.irfft(fft_signal, n=len(gpu_milli_data))
...
class GRUModel(nn.Module):
    def __init__(self, input_size=3, hidden_size=128, num_layers=1, forecast_horizon=1):
...
plt.title('GRU Usage Prediction (FFT)')
plt.xlabel('Time (sec)')
