...
def create_multistep_dataset(data, lookback_time=20, forecast_horizon=1):
    ...
    for i in range(len(data) - lookback_time - forecast_horizon + 1):
        X.append(data[i:i + lookback_time])
        y.append([data[i + lookback_time + j][0] for j in range(forecast_horizon)])
    X = np.array(X)
    y = np.array(y)
...
def train(local_rank, rank, world_size, epochs=50):
    ...
    for epoch in range(epochs):
        ...
        print(f"[Rank {rank}] Epoch [{epoch+1}/{epochs}] - Train Loss: {total_loss/len(train_loader):.6f}, Validation Loss: {val_loss:.6f}")
